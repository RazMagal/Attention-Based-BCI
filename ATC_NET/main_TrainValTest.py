...
def draw_learning_curves(history, sub):
    ...
    plt.xlabel('Epoch')
    plt.legend(['Train', 'val'], loc='upper left')
    plt.savefig('loss_subject_' + str(sub) + '.png')
    print('saved graphs for subject: ' + str(sub))
    plt.close()
...
def train(dataset_conf, train_conf, results_path):
    ...
    for sub in range(n_sub): # (num_sub): for all subjects, (i-1,i): for the ith subject.
        ...
        for train in range(n_train): # How many repetitions of training for subject i.
            ...
            print("got to Training!",flush=True)

            # X_train (230, 1, 22, 1125), X_val (58, 1, 22, 1125)
            # y_train (230, 4),           y_val (58, 4)
            ...
            print("Validation set class distribution:", Counter(np.argmax(y_val_onehot, axis=1)))

            history = model.fit(X_train, y_train_onehot, validation_data=(X_val, y_val_onehot),
                                epochs=epochs, batch_size=batch_size, callbacks=callbacks, verbose=0)

            loss, accuracy, precision, recall = model.evaluate(X_val, y_val_onehot)
...
def test(model, dataset_conf, results_path, allRuns = True):
    ...
    for sub in range(n_sub): # (num_sub): for all subjects, (i-1,i): for the ith subject.
        ...
        for seed in range(len(runs)): 
            ...
            # Map the original 4 labels to 2 labels
            #y_pred = map_labels(y_pred, label_mapping)
            #labels = map_labels(y_test_onehot.argmax(axis=-1), label_mapping)
...
def run():
    ...
    dataset = 'ELIS' # Options: 'BCI2a','HGD', 'CS2R'
    sub_mapping=[]
    ...
    if dataset == 'ELIS':
        ...
        data_path = os.path.expanduser('~') + '/dataset/'
    elif dataset == 'HGD': 
        in_samples = 1125
        n_channels = 44
        n_sub = 14
    ...
    dataset_conf = { 'name': dataset, 'n_classes': n_classes, 'cl_labels': classes_labels,
                    ...
                    'data_path': data_path, 'isStandard': True, 'LOSO': False, 'ELIS_sub_mapping':sub_mapping}
